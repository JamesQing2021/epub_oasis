...
import '/src/data/epub_cfi_reader.dart';
import '/src/data/epub_parser.dart';
import '/src/data/models/chapter.dart';
import '/src/data/models/chapter_view_value.dart';
import '/src/data/models/paragraph.dart';
...
class EpubOasisView extends StatefulWidget {
  const EpubOasisView({
  ...
  @override
  State<EpubOasisView> createState() => _EpubOasisViewState();
}

class _EpubOasisViewState extends State<EpubOasisView> {
  ...
  void initState() {
    ...
    _controller.loadingState.addListener(() {
      ...
      SharedText.actions = widget.actions!;
    });
  }
      ...
      children: <Widget>[
        ...
        SelectionArea(
          contextMenuBuilder: (
            ...
            var list = <ContextMenuButtonItem>[];
            if (SharedText.actions.isNotEmpty) {
              for (var i = 0; i < SharedText.actions.length; i++) {
                list.add(ContextMenuButtonItem(
                  onPressed: () {
                    ContextMenuController.removeAny();
                    // _showDialog(context);
                    debugPrint(SharedText.selectedText);
                    SharedText.actions[i].action(SharedText.selectedText);
                  },
                  label: SharedText.actions[i].label,
                ));
              }
            }
          ...
          onSelectionChanged: (value) {
            if (value?.plainText == null) return;
            SharedText.setText(value?.plainText);
          },
          child: Html(
...
class SharedText {
  ...
  static List<CustomButton> actions = [];

  static String selectedText = '';

  static setText(String? val) {
    selectedText = val!;
  }
}
...
class CustomButton {
  ...
  const CustomButton({required this.label, required this.action});
}
